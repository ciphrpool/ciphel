...
use super::{
    Address, Closure, Data, Enum, ExprScope, FieldAccess, Generator, ListAccess, Map, NumAccess,
...
impl<Scope: ScopeApi> GenerateCode<Scope> for Data<Scope> {
    fn gencode(
        ...
        match self {
            ...
            Data::Generator(value) => value.gencode(scope, instructions),
        }
    }
}
...
impl<Scope: ScopeApi> GenerateCode<Scope> for Generator<Scope> {
    fn gencode(
        &self,
        scope: &MutRc<Scope>,
        instructions: &CasmProgram,
        ...
        let gen_next_label = Label::gen();
        let yield_label = Label::gen();
        let end_label = Label::gen();

        instructions.push(Casm::Goto(Goto {
            label: Some(end_generator),
        }));

        let init_label = instructions.push_label("init_generator_label".into());
        let _ = self.iterator.expr.gencode(scope, instructions)?;
        let generator_label = Label::gen();
        /* init itertor index */
        let _ = iterator_type.init_index(instructions)?;

        /* push LABEL_GEN_BUILD_ITEM onto the stask */
        instructions.push(Casm::MemCopy(MemCopy::LabelOffset(generator_label)));

        instructions.push(Casm::MemCopy(MemCopy::GetReg(UReg::R2)));
        instructions.push(Casm::Goto(Goto { label: None }));

        instructions.push_label_id(generator_label, "loop_generator".into());
        // instructions.push(Casm::StackFrame(StackFrame::OpenWindow));

        instructions.push_label_id(start_label, "start_for_generator".into());
        ...
        instructions.push(Casm::MemCopy(MemCopy::LabelOffset(yield_label)));
        ...
        instructions.push_label_id(gen_next_label, "gen_next_label".into());
        let _ = iterator_type.next(instructions)?;
        instructions.push(Casm::MemCopy(MemCopy::LabelOffset(generator_label)));
        instructions.push(Casm::MemCopy(MemCopy::GetReg(UReg::R1)));
        instructions.push(Casm::Goto(Goto { label: None }));

        instructions.push_label_id(yield_label, "yield_label".into());
        instructions.push(Casm::StackFrame(StackFrame::LastYield {
            next_label: gen_next_label,
        }));

        instructions.push_label_id(end_label, "end_for_generator".into());
        // instructions.push(Casm::StackFrame(StackFrame::CloseWindow));
        instructions.push(Casm::StackFrame(StackFrame::LastYieldNone));

        instructions.push_label_id(end_generator, "end_generator".into());

        instructions.push(Casm::MemCopy(MemCopy::LabelOffset(init_label)));
        // instructions.push(Casm::MemCopy(MemCopy::LabelOffset(generator_label)));
...
mod tests {
    ...
    fn valid_generator() {
        ...
        .expect("Deserialization should have succeeded");
        assert_eq!(result, Primitive::Number(Cell::new(Number::I64(6))));
    }

    #[test]
    fn valid_generator_different_index_size() {
        let statement = Statement::parse(
            r##"
                let x = {
                    let gen = for i in 0u8..3u8 {
                        yield i * 2u8;
                    };
                    let res = 0u8;
                    for i in gen {
        ...
        .expect("Deserialization should have succeeded");
        assert_eq!(result, Primitive::Number(Cell::new(Number::U8(6))));
    }

    #[test]
    fn valid_generator_slice() {
        let statement = Statement::parse(
            r##"
                let x = {
                    let tab = [5,10,15];
                    let gen = for i in tab {
                        yield i * 2;
                    };
                    let res = 0;
                    for i in gen {
        ...
        .expect("Deserialization should have succeeded");
        assert_eq!(result, Primitive::Number(Cell::new(Number::I64(60))));
    }

    #[test]
    fn valid_generator_vec() {
        let statement = Statement::parse(
            r##"
                let x = {
                    let tab = vec[5,10,15];
    ...
    fn valid_generator_filter() {
        ...
        .expect("Deserialization should have succeeded");
        assert_eq!(result, Primitive::Number(Cell::new(Number::I64(4))));
    }

    #[test]
    fn robustness_generator_empty() {
        let statement = Statement::parse(
            r##"
                let x = {
                    let gen = for i in 0..3 {
        ...
        .expect("Deserialization should have succeeded");
        assert_eq!(result, Primitive::Number(Cell::new(Number::I64(0))));
    }
